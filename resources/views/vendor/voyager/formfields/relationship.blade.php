@if (isset($options->model) && isset($options->type))
    ...
    @if (class_exists($options->model))
        ...
        @if ($options->type == 'morphMany')
                    ...
                    $selected_values = isset($relationshipData)
                        ? $relationshipData
                            ...
                            ->map(function ($item, $key) use ($options) {
                                $filename = Storage::disk(config('voyager.storage.disk'))->path($item->{$options->label});
            ...
            @else
                @php
                    $items = $dataTypeContent->morphMany($options->model, 'entity')->orderBy('sort','asc')->get();
                    $unqid = 's' . uniqid();
                @endphp
                @if ($items != null)
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:1rem" id={{ $unqid }}>
                            @foreach ($items as $item)
                                ...
                                <div class="img_settings_container" data-field-name="{{ $row->field }}"
                                    ...
                                    <a href="javascript:;" class="voyager-x" onclick="$(this).closest('.img_settings_container').remove()"
