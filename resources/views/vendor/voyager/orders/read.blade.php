    ...
    <div class="page-content read container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-bordered" style="padding-bottom:5px;">
                    <div class="panel-heading" style="border-bottom:0;">
                        <h3 class="panel-title">Заказ №{{ $dataTypeContent->id }} от
                            {{ $dataTypeContent->created_at->format('d.m.Y H:i') }}</h3>
                    </div>
                    {{-- region, city, street, house, appartement  --}}
                    <div class="panel-body" style="padding-top:0;">

                        <h4 class="py-2">
                            {{ $dataTypeContent->name }}
                            {{ $dataTypeContent->lastname }},
                            {{ $dataTypeContent->phone }},
                            {{ $dataTypeContent->email }}
                        </h4>
                        <p class="">
                            @if ($dataTypeContent->region)
                                {{ $dataTypeContent->region }},
                            @endif
                            @if ($dataTypeContent->city)
                                {{ $dataTypeContent->city }},
                            @endif
                            @if ($dataTypeContent->street)
                                {{ $dataTypeContent->street }},
                            @endif
                            @if ($dataTypeContent->house)
                                {{ $dataTypeContent->house }},
                            @endif
                            @if ($dataTypeContent->appartement)
                                {{ $dataTypeContent->appartement }}
                            @endif
                        </p>
                        @if ($dataTypeContent->comment)
                            <p class="py-2">
                                {{ $dataTypeContent->comment }}
                            </p>
                        @endif
                        @foreach ($dataTypeContent->cart->items as $item)
                            <p class="">
                                {{ $item->offer->title }}, {{ $item->quantity }} x {{ $item->price }}₸
                            </p>
                        @endforeach
                        <h5 class="">
